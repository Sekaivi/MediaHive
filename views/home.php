...
<div id="trendingArticles" style="display:flex; flex-wrap:wrap; gap:20px; padding:20px;">
  <?php foreach ($trendingArticles as $index => $article): ?>
    <div id="<?= htmlspecialchars($article['articleID']) ?>" class="article-card">
      <!-- Image centrée -->
      <img class="article-image" onerror="this.style.display='none';" src="<?= $article['image'] ?>" alt="<?= htmlspecialchars($article['title']) ?>">
      ...
      <div class="article-content">
        ...
        <p class="article-description"><?= substr($article['description'], 0, 100) . (strlen($article['description']) > 100 ? '...' : '') ?>        </p>
      </div>
