...
function updateIndicators(carouselIdSuffix) {
  const carousel = document.getElementById("carousel-" + carouselIdSuffix);
  const currentIndex = parseInt(carousel.dataset.currentIndex) || 0;
  const indicators = document.getElementById("indicators-" + carouselIdSuffix).children;
  for (let i = 0; i < indicators.length; i++) {
    indicators[i].classList.toggle("active", i === currentIndex);
  }
}

function nextSlide(carouselIdSuffix) {
  const carousel = document.getElementById("carousel-" + carouselIdSuffix);
  ...
  updateIndicators(carouselIdSuffix);
}

function prevSlide(carouselIdSuffix) {
  const carousel = document.getElementById("carousel-" + carouselIdSuffix);
  let currentIndex = parseInt(carousel.dataset.currentIndex) || 0;
  const slides = carousel.children;
  const slideCount = slides.length;
